...
public class BackendHelper {
    ...
    public static BackendService mService;
                ...
                if (message.length() > 4000) {
                    for (int i = 0; i < message.length(); i += 4000) {
                        if (i + 4000 < message.length())
                            Log.i("BackendHelper" + i, message.substring(i, i + 4000));
                        else
                            Log.i("BackendHelper" + i, message.substring(i, message.length()));
                    }
                } else {
                    Log.i("BackendHelper", message);
                }
    ...
    public static BackendService getService() {
        if(mService == null) {
            mService = createService(BackendService.class);
        }
        return mService;
